...
- (void)layoutSubviews {
    ...
    if(hasCustomView) {
        ...
        CGRect titleRect = [self.titleLabel.text boundingRectWithSize:CGSizeMake(labelMaxWidth,self.titleLabel.font.lineHeight) options:NSStringDrawingUsesLineFragmentOrigin attributes:@{NSFontAttributeName : self.titleLabel.font} context:nil];
        
        CGRect subtitleRect = [self.subtitleLabel.text boundingRectWithSize:CGSizeMake(labelMaxWidth, self.subtitleLabel.font.lineHeight) options:NSStringDrawingUsesLineFragmentOrigin attributes:@{NSFontAttributeName : self.subtitleLabel.font} context:nil];
...
//                                                      lineBreakMode:self.subtitleLabel.lineBreakMode];
        ...
        CGFloat maxLabelWidth = MAX(titleRect.size.width,subtitleRect.size.width);
        ...
        if(subtitleRect.size.height > 0){
            CGFloat totalHeight = titleRect.size.height + subtitleRect.size.height + marginY;
            CGFloat minY = (self.bounds.size.height / 2)  - (totalHeight / 2);
            
            CGFloat titleY = minY;
            self.titleLabel.frame = CGRectIntegral(CGRectMake(labelX, titleY, titleRect.size.width, titleRect.size.height));
            self.subtitleLabel.frame = CGRectIntegral(CGRectMake(labelX, titleY + titleRect.size.height + marginY, subtitleRect.size.width, subtitleRect.size.height));
        }else{
            CGFloat totalHeight = titleRect.size.height;
            CGFloat minY = (self.bounds.size.height / 2)  - (totalHeight / 2);
